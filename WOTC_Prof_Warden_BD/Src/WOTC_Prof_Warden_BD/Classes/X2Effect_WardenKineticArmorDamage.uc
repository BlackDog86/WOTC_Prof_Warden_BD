...
function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState)
{
	...
	`log("Kinetic Armor Bonus Damage UnitValue = " @UV.fValue,,'BDLOG');
	// If the source weapon exists
	if (SourceWeapon != none)
	{
		// Bonus damage = amount of shields lost / shielding factor, rounding up damage to weapon
		BonusDamage = Round((UV.fValue)*((float(default.KINETIC_ARMOR_SHIELD_DAMAGE_PERCENTAGE)) / 100.0f));			
		`log("Bonus damage to apply = " @BonusDamage,,'BDLOG');
				...
				if (DamageEffect != none && !DamageEffect.bIgnoreBaseDamage)
				{
					...
					`log("Kinetic Shot taken - Resetting Bonus Damage Unitvalue to 0",,'BDLOG');
